...
const valid = {
  ...
  created_by_attempt: "attempt-1",
  input_artifacts: ["code-v1"],
  validation_status: "passed",
};
const relative = "artifact-version.schema.json";

test("artifact-version.schema.json: valid sample passes", () => {
...
test("artifact-version.schema.json: missing required and extra property fail", () => {
  const schemaPath = path.join(schemaRoot, relative);
...
test("relative artifact paths reject absolute, parent-escaping, drive-relative and Windows paths", () => {
  const schemaPath = path.join(schemaRoot, "artifact-version.schema.json");
  const schema = loadSchema(schemaPath);
  for (const pathValue of [
    ...
    assert.throws(
      () => validate({ ...valid, path: pathValue }, schema, schemaPath),
      /invalid format/,
    );
  }
});

test("artifact-version.schema.json: enum constraints reject invalid kind", () => {
  const schemaPath = path.join(schemaRoot, relative);
  assert.throws(
    () =>
      validate(
