...
const valid = {
  ...
  template_version: "v1",
  input_version: "v1",
  input_artifacts: ["problem-pdf"],
};
const relative = "commands/command.schema.json";

test("commands/command.schema.json: valid sample passes", () => {
...
test("commands/command.schema.json: missing required and extra property fail", () => {
  const schemaPath = path.join(schemaRoot, relative);
...
test("command variants require idempotency keys and reject invalid actions", () => {
  const schemaPath = path.join(schemaRoot, "commands/command.schema.json");
  const schema = loadSchema(schemaPath);
  const variants = [
    valid,
    {
    ...
    { command: "cancel_run", idempotency_key: "request-4", run_id: "run-1" },
  ];
  for (const sample of variants)
    assert.doesNotThrow(() => validate(sample, schema, schemaPath));
  const missingKey = { ...variants[1] };
  delete missingKey.idempotency_key;
  assert.throws(
    () => validate(missingKey, schema, schemaPath),
    /missing required|oneOf branch/,
  );
  assert.throws(
    () =>
      validate({ ...variants[1], action: "approve_now" }, schema, schemaPath),
    /oneOf branch|one of/,
  );
});
