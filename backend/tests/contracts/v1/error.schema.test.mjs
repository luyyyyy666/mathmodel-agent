...
const valid = {
  ...
  details: {
    step_id: "step-1",
    attempt: 2,
    hint: "retry with a larger budget",
  },
};
const relative = "error.schema.json";

test("error.schema.json: valid sample passes", () => {
...
test("error.schema.json: missing required and extra property fail", () => {
  const schemaPath = path.join(schemaRoot, relative);
...
test("error.schema.json: enum constraints reject invalid code", () => {
  const schemaPath = path.join(schemaRoot, relative);
  assert.throws(
    () =>
      validate(
