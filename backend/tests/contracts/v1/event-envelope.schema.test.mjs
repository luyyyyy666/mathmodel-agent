...
const relative = "event-envelope.schema.json";

test("event-envelope.schema.json: valid sample passes", () => {
...
test("event-envelope.schema.json: missing required and extra property fail", () => {
  const schemaPath = path.join(schemaRoot, relative);
...
test("event-envelope.schema.json: enum constraints reject invalid type", () => {
  const schemaPath = path.join(schemaRoot, relative);
  assert.throws(
    () =>
      validate(
