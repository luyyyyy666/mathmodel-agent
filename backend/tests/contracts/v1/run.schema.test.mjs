...
const valid = {
  ...
  updated_at: "2026-09-09T01:02:04+08:00",
};
const relative = "run.schema.json";

test("run.schema.json: valid sample passes", () => {
...
test("run.schema.json: missing required and extra property fail", () => {
  const schemaPath = path.join(schemaRoot, relative);
...
test("run.schema.json: enum constraints reject invalid status", () => {
  const schemaPath = path.join(schemaRoot, relative);
  assert.throws(
    () =>
      validate(
