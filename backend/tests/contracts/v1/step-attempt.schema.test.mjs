...
const valid = {
  ...
  status: "completed",
  executor: "planner",
  input_artifacts: ["analysis-v1"],
  output_artifacts: ["model-v1"],
  started_at: "2026-09-09T01:02:03Z",
  completed_at: "2026-09-09T01:02:04Z",
  error: null,
};
const relative = "step-attempt.schema.json";

test("step-attempt.schema.json: valid sample passes", () => {
...
test("step-attempt.schema.json: missing required and extra property fail", () => {
  const schemaPath = path.join(schemaRoot, relative);
...
test("step-attempt.schema.json: enum constraints reject invalid step_name", () => {
  const schemaPath = path.join(schemaRoot, relative);
  assert.throws(
    () =>
      validate(
        { ...valid, step_name: "unknown" },
